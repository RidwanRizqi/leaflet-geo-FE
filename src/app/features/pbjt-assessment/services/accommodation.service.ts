...
export class AccommodationService {
  ...
  /**
   * Get all accommodations from backend with optional search
   */
  getAccommodations(filter?: AccommodationFilter): Observable<PaginatedResponse<Accommodation>> {
    ...
    // Use /with-realization to get actual revenue/tax data from realisasi table
    return this.http.get<any>(`${this.apiUrl}/with-realization`, { params }).pipe(
      map(response => {
        ...
        // Apply client-side filters not handled by backend
        let filtered = items;
  ...
  /**
   * Create accommodation in LOCAL database
   */
  ...
  /**
   * Update accommodation in LOCAL database
   */
  ...
  /**
   * Delete accommodation from LOCAL database
   */
  ...
  /**
   * Sync data FROM SIMATDA (read-only import)
   */
  ...
  /**
   * Get hotels with realization data
   */
  ...
  private mapToAccommodation(data: any): Accommodation {
    // Calculate revenue/tax from realization data if available
    ...
    // Use realization to estimate revenue (tax is ~10% of revenue)
  ...
  /**
   * Parse PostgreSQL text[] array literal into string array.
   * Handles quoted elements with commas/special chars.
   * Input: {"/path,with,comma","simple/path"}
   */
  private parsePostgresArray(value: any): string[] {
    ...
    while (i < inner.length) {
      if (inner[i] === '"') {
        // Quoted element — find matching closing quote
        i++;
        let end = i;
        ...
        if (commaIdx === -1) {
          result.push(inner.slice(i).trim());
          break;
        }
