...
import { BprdApiService } from '../../../../services/bprd-api.service';
...
export class PbjtMapComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    if (url.startsWith('/pbjt-hotel')) {
      this.category = 'hotel';
    } else if (url.startsWith('/pbjt-makanan')) {
  ...
  loadKecamatanStats(): void {
    ...
    this.pbjtService.getStatsByKecamatan(this.category).subscribe({
      next: (stats) => {
  ...
  loadKelurahanStats(kecamatan: string): void {
    this.isLoadingStats = true;
    this.pbjtService.getStatsByKelurahan(kecamatan, this.category).subscribe({
      next: (stats) => {
  ...
  loadBusinessMarkers(kecamatan: string, kelurahan: string): void {
    ...
    this.pbjtService.getAssessmentsByLocationWithRealization(kecamatan, kelurahan).subscribe({
      ...
      error: (error) => {
        ...
        this.pbjtService.getAssessmentsByLocation(kecamatan, kelurahan, this.category).subscribe({
          next: (assessments) => {
            this.assessments = assessments;
            this.renderBusinessMarkers(assessments);
          },
